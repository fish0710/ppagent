...
| `loop.maxTurns` | `60` | `PPAGENT_MAX_TURNS` | `--max-turns` | `0` 表示不限制轮数，只能以 stop/error/aborted 退出；模型持续产出工具调用时可持续运行，由根取消或模型自身的 stop/error 终结 |
| `loop.turnTimeoutMs` | `1200000`（20 分钟） | `PPAGENT_TURN_TIMEOUT_MS` | `--turn-timeout-ms` | 整轮编排超时，含工具执行；`0` 表示不限时；见下方两种超时的区分 |
...
把它设成 `0` 可以完全关闭单轮 deadline——此时一轮可能无限挂起，只能靠根取消或工具自身的
超时兜底，谨慎使用。
