...
def deep_find_key(obj, names, _depth=0):
    """在嵌套 dict/list 里广度优先找第一个命中的键，连键名一起返回。
    键名本身携带单位信息时（比如 durationMs 的 "Ms"），只看数值猜单位会出错。"""
    if _depth > 8:
        return None, None
    if isinstance(obj, dict):
        for n in names:
            if n in obj and obj[n] is not None:
                return n, obj[n]
        for v in obj.values():
            k, r = deep_find_key(v, names, _depth + 1)
            if r is not None:
                return k, r
...
def deep_find(obj, names, _depth=0):
    ...
    return deep_find_key(obj, names, _depth)[1]
...
def cmd_calibrate(args):
    ...
    # estimate 只消费 decode_tps_short / decode_tps_long / prefill_curve 三项，
    # 是它公式里 T_decode / T_prefill 的直接输入。别的量测都不产出 estimate 会用的数字。
    print("== 1/3 decode 速率（短上下文）==")
    r = call("decode_short",
    ...
    print("== 3/3 decode 速率（长上下文，KV 压力下）==")
...
def trial_name(path):
    """从 events.jsonl 路径反推 trial 目录名。

    真实路径形如 jobs/<job>/<task>__<hash>/agent/ppagent/events.jsonl——
    "ppagent" 是 benchmark/harbor/ppagent.py 里 _EVENTS_FILENAME 定死的子目录，
    "agent" 是 Harbor 自己的日志布局，两层都是与题目无关的固定名字，
    往上跳过它们才是真正的 <task>__<hash> 目录。
    """
    d = path.parent
    while d.name in ("ppagent", "agent") and d.parent != d:
        d = d.parent
    return d.name
...
def parse_trial(path):
    ...
    for e in evs:
        ...
        if "turn_end" in t or "turn_complete" in t or t == "usage":
            ...
            m["tool_calls"] += 1
            dur_key, dur = deep_find_key(e, KEYS_DURATION)
            if isinstance(dur, (int, float)):
                d = float(dur)
                # 键名写明单位就按键名来，不按量级猜——ppagent 真实事件里的
                # durationMs 常年 <1000（大多数工具调用不到一秒），量级猜测法
                # 会把「850 毫秒」误判成「850 秒」，T_tool 直接虚高两个数量级。
                key_lower = (dur_key or "").lower()
                if "ms" in key_lower:
                    d = d / 1000.0
                elif "sec" in key_lower:
                    pass
                else:
                    d = d / 1000.0 if d > 1000 else d
                m["T_tool"] += d
                continue
            cid = deep_find(e, KEYS_CALLID)
            start = None
...
def cmd_estimate(args):
    ...
    print("\n=== 敏感性（中位口径，均为跑完全量；只列不需要臆测硬件参数的场景）===")
    scen = [
...
def main():
    ...
    c = sub.add_parser("calibrate", help="微基准：decode/prefill")
